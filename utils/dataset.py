...
class IntentScenarioDataset:
    def __init__(self, text, intent, scenario, require_text=False):
        self.texts = text
        self.intent = intent
    ...
    def __getitem__(self, item):
        ...
        if self.require_text:
            out = self.tokenizer.encode_plus(
                text, None, add_special_token=True, max_length=self.max_len, padding='max_length')
            ids = out['input_ids']
...
class NLUDataset(torch.utils.data.Dataset):
    def __init__(self, text, entity, intent, scenario):
        ...
        self.entity_dataset = EntityDataset(self.texts, self.entity)
        self.intent_scenario_dataset = IntentScenarioDataset(
            self.texts, self.intent, self.scenario, require_text=False)
...
if __name__ == "__main__":
    test_text = [['hello', 'I', 'am', 'chandra']]
    test_entity = [[3, 1, 1, 4]]
    test_intent, test_scenario = [[[3]], [1]]
    ...
    is_dataset = IntentScenarioDataset(
        test_text, test_intent, test_entity, require_text=False)
    out = is_dataset[0]

    nlu_dataset = NLUDataset(test_text, test_entity,
                             test_intent, test_scenario)
    out = nlu_dataset[0]
